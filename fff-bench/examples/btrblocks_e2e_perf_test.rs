...
use anyhow::Result;
use fff_bench::bench_data::{BenchmarkDataset, TPCHDataset};
use fff_bench::bench_data::{
    BenchmarkDatasets::{CFB, TPCH},
    CFBDataset,
};
#[tokio::main]
async fn main() -> Result<()> {
    let guard = pprof::ProfilerGuardBuilder::default()
        ...
        .unwrap();
    for _ in 0..20 {
        // TPCH(TPCHDataset::Lineitem).read_btrblocks().unwrap();
        CFB(CFBDataset::Log).read_btrblocks().unwrap();
    }
    if let Ok(report) = guard.report().build() {
        let file = File::create("btr_log.svg").unwrap();
